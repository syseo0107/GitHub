...
Mock.onGet("/api/tableData1/all").reply((config: any) => {
  return [200, tableData1];
});

Mock.onPost("/api/tableData1/new").reply((config: any) => {
...
Mock.onPost("/api/tableData1/delete").reply((config: any) => {
...
Mock.onPost("/api/tableData2/new").reply((config: any) => {
  const { name, username, email, position, team, experience, dateOfBirth, address, status } =
...
Mock.onPost("/api/tableData2/delete").reply((config: any) => {
  const { ids } = JSON.parse(config.data);
