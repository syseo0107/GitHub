import { Card, Table, TableHead, TableRow, Theme } from "@mui/material";
...
const RecentDevices: FC = () => {
  const tableHeadColor = (theme: Theme) => (lightTheme(theme) ? "primary.100" : "divider");

  return (
    <Card sx={{ pb: 1 }}>
      <FlexBetween padding={3} flexWrap="wrap">
        <H5 lineHeight={1.8}>Recent Devices</H5>
        <Tiny lineHeight={1.5}>View and manage devices where you're currently logged in.</Tiny>
      </FlexBetween>

      <Box>
        <Scrollbar autoHide={false}>
          <Table sx={{ minWidth: 800 }}>
            <TableHead sx={{ backgroundColor: (theme: Theme) => tableHeadColor(theme) }}>
              <TableRow>
                ...
                <TableRow key={item.id}>
                  <BodyTableCell>
                    <FlexBox alignItems="center">
                      <AppAvatar src={item.browserIcon} sx={{ width: 20, height: 20, mr: 1 }} />
                      <H6>{item.browser}</H6>
                    </FlexBox>
                  </BodyTableCell>
