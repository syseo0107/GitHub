...
const TeamProgress: FC = () => {
  ...
  const { getTableProps, getTableBodyProps, headerGroups, prepareRow, rows }: any = useTable(
    { columns, data },
    useRowSelect
  );
...
const columnShape = [
  {
    ...
    Cell: ({ value }: any) => (
      <FlexBox alignItems="center">
        <AppAvatar src={value.image} alt={value.name} sx={{ borderRadius: "15%" }} />
  ...
  },
  {
    ...
    Cell: ({ value }: any) => {
      return (
        <LinearProgress variant="determinate" value={value} sx={{ height: 5, borderRadius: 5 }} />
      );
    },
  },
  {
